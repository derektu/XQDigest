...
const inputStyle = { width: '100%', padding: '6px 10px', borderRadius: 4, border: '1px solid #ccc', fontSize: 14, boxSizing: 'border-box' };
const btnRow = { display: 'flex', justifyContent: 'flex-end', gap: 8, marginTop: 20 };
const btnPrimary = { padding: '7px 18px', borderRadius: 4, border: 'none', background: '#0d6efd', color: '#fff', cursor: 'pointer', fontSize: 14 };
const btnSecondary = { padding: '7px 18px', borderRadius: 4, border: '1px solid #ccc', background: '#fff', cursor: 'pointer', fontSize: 14 };
...
export default function DataSourceForm({ initial, onSave, onCancel, onValidate }) {
  ...
  const [forceSaveConfirm, setForceSaveConfirm] = useState(null); // null | { error: string }
  ...
  // Auto-generate ID from type + name when adding
  useEffect(() => {
    if (!isEdit && !idManual) {
  ...
  const handleSave = async () => {
    ...
    // Auto-validate if not yet validated and URL is provided
  ...
  return (
    <div style={formStyle} onClick={onCancel}>
      <div style={modalStyle} onClick={(e) => e.stopPropagation()}>
        <h3 style={{ margin: '0 0 16px' }}>{isEdit ? '編輯資料源' : '新增資料源'}</h3>

        {errors._general && <div style={{ color: '#dc3545', marginBottom: 12, fontSize: 13 }}>{errors._general}</div>}
        ...
        <div style={fieldStyle}>
          ...
          />
          {errors.id && <span style={{ color: '#dc3545', fontSize: 12 }}>{errors.id}</span>}
        </div>
        ...
        <div style={fieldStyle}>
          ...
          {errors.url && <span style={{ color: '#dc3545', fontSize: 12 }}>{errors.url}</span>}
          {form.type === 'youtube' && (
        ...
        <div style={{ display: 'flex', gap: 12 }}>
          <div style={{ ...fieldStyle, flex: 1 }}>
            ...
            {errors.checkInterval && <span style={{ color: '#dc3545', fontSize: 12 }}>{errors.checkInterval}</span>}
          </div>
          <div style={{ ...fieldStyle, flex: 1 }}>
            ...
            {errors.maxItems && <span style={{ color: '#dc3545', fontSize: 12 }}>{errors.maxItems}</span>}
          </div>
          <div style={{ ...fieldStyle, flex: 1 }}>
            ...
            {errors.lookbackDays && <span style={{ color: '#dc3545', fontSize: 12 }}>{errors.lookbackDays}</span>}
          </div>
        </div>
